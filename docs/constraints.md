...
7. Windows launch path: **built** (`launchers/start-spad-windows.bat` + `-practice.bat`, guide in `docs/windows_smoke_test.md`; `frontend/dist` is committed so the host needs Python only, no Node). The `.bat` files themselves are untested until they run on a real Windows machine — that's Step 3 of the guide.
...
9. **Paced gated decomposition** (Phase 16): `cooloff_s > 0` replaces one `G` command with one single-step `G` per gate offset. Validate on hardware that per-command overhead is acceptable and that stepping `gate_offset` across separate commands is optically equivalent to the vendor's internal sweep (same gate timing per step).
