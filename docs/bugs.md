...
| B-38 | S1 | **First lab smoke test (Windows):** every acquisition failed with `acquisition length mismatch: expected 262144, got 262170` — a stray 26-byte text reply (`"Pileup correction off\nDONE"`) glued onto the frame data. Root cause: the client's idle EOF watcher `read(1)` silently swallowed any byte arriving between commands, permanently desyncing the strict lockstep stream; a wrong-length payload then failed forever (no recovery). Fixed by making both desync symptoms self-heal via reconnect: idle-arriving data now drops the connection immediately (framing is broken by definition), and a wrong-length acquisition resets the connection before surfacing its `ProtocolError`. | hardware smoke-test fixes | `tests/test_protocol_desync.py` (2: stray-prefix length mismatch → reset + recover; unsolicited idle byte → reconnect + next acquire clean) |
