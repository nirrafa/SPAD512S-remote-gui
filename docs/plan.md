...
| 2026-06-28 | FLIM command format aligned to `cSPAD.py` (no comma after `c`/`i`, includes `intTime`) | `python_tcp_stream_flim.py` disagrees (comma form); code follows the primary client + learnings.md — validate on hardware (B-17) |
| 2026-06-29 | Raw 1-bit reuses the intensity path (`bit_depth=1`); no separate `raw_1bit.py` service | The 1-bit decode + intensity acquire already do the binary unpacking; a service module would be dead indirection (B-06 lesson) |
| 2026-06-29 | Calibration store is in-memory per-app (`core/calibration_state.py`); breakdown marked `done` on connect | Vendor connect handshake performs breakdown; SQLite persistence deferred to Phase 12 |
| 2026-06-30 | Auto-abort on a single socket needs a mid-run boundary → **batch multi-iteration intensity** (`BATCH_SIZE=10`); health polled between batches | Only way to poll temps mid-acquisition without corrupting the stream; gated/FLIM remain single-command (B-32) |
| 2026-06-30 | Auto-protect + health live in `HealthMonitor`, not a separate `auto_protect.py` | One owner of the read → evaluate → protect cycle |
| 2026-06-30 | Mock extends `R` with `cooling,saturated` (8 fields) so one poll covers all health | Real vendor returns 6 → those two alarms are mock-only until hardware (B-34/B-35); a dedicated health command would be cleaner |
| 2026-07-01 | **Bridge serves the built SPA at `/`** (StaticFiles, same origin :8080); launcher builds once, no Vite at runtime | The dev-server launch path was fragile (readiness race, npm PATH on double-click, port confusion) → blank page for the user |
| 2026-07-04 | Sweep runner reuses `AcquisitionRunner`; SQLite checkpoints under `data_root`; `/resume` skips done points | Survives browser disconnect; no acquire-lifecycle duplication |
