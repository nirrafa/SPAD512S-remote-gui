...
**Phase(s):** 9
**Duration:** ~3h across session-limit interruptions
**Who:** Nir + Claude (Opus agent, resumed twice; finished inline)

#### Done
- **Bridge:** `services/sweep.py` (sweep runner reusing AcquisitionRunner; per-point labels; SQLite checkpoints via `core/checkpoint.py` under `data_root`; `POST /api/acquire/sweep` + `/resume` skips completed points; survives browser disconnects; progress via `GET /api/acquire/status`), `services/scheduler.py` (asyncio scheduled jobs: `POST /api/acquire/schedule`, `GET /api/acquire/schedule/<id>`), `services/experiment_log.py` + `routes/experiments.py` (scheduled jobs visible in log), safe-boundary stop between sweep points.
- **Front-end:** Sweep tab — `SweepPage`, `SweepConfig`, `SweepProgress`, `ScheduleForm`.
- **Tests:** test_06 14/14, test_12 9/9; 139-test regression green; full pre-dev 149/202 (remainder is Phases 10–13). ruff + mypy clean; frontend build/lint/test green.

#### Next session
- Phase 10 (data handling & reducer integration), then the post-review fix pass (stop endpoint / STOPPING busy hole / auto-protect Vex — see bugs.md + task list).
